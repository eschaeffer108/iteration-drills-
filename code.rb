# Write Ruby<->English answers here as comments in your code

# For each number in the array called "numbers" multiply the number by 3
# For each element called "name" in the array "names" print the character length of each name
# For each number in the "numbers" array add up all the numbers in the array and print the sum
# For each key and value called "name" and "age" respectively in the hash called "hash" write a sentance saying "(name) is (age) years old."
# For each key and value called "transaction" and "value" in the hash called "accountant" add up the sum of each value and write a sentance saying "The value of the account is (sum)"
# For each key and value called "name" and "address" in the hash called "address" write a sentance saying "(name) lives at (address)"

# sentances.each do |word|
# puts word
# end

# numbers.each do |phone_number|
# if (617).includes phone_number
# puts phone_number
# end

# numbers.each do |number|
# if number % 2 != 0
# puts number
# end

# ages.each do |name, age|
# puts "#{name} is #{age} years old."
# end

# ages.each do |name, age|
# if age > 10
# puts "#{name} is #{age} years old."
# end
# end

# ages.each do |name, age|
# if age % 2 == 0
# puts "#{name} is #{age} years old."
# end
# end

array = [28214, 63061, 49928, 98565, 31769, 42316, 23674, 3540, 34953, 70282, 22077, 94710, 50353, 17107, 73683, 33287, 44575, 83602, 33350, 46583]

# Write Ruby code to find out the answers to the following questions:

# * What is the sum of all the numbers in `array`?
sum = 0
array.each do |number|
  sum += number
end
puts sum

# * How would you print out each value of the array?
array.each do |number|
puts number
end

# * What is the sum of all of the even numbers?
sum = 0
array.each do |number|
  if number % 2 == 0
    sum += number
  end
end
puts sum
# * What is the sum of all of the odd numbers?
sum = 0
array.each do |number|
  if number % 2 != 0
    sum += number
  end
end
puts sum
# * What is the sum of all the numbers divisble by 5?
sum = 0
array.each do |number|
  if number % 5 == 0
    sum += number
  end
end
puts sum
# * What is the sum of the squares of all the numbers in the array?
sum = 0
array.each do |number|
  sum += number * number
end
puts sum
array = ["joanie", "annamarie", "muriel", "drew", "reva", "belle", "amari", "aida", "kaylie", "monserrate", "jovan", "elian", "stuart", "maximo", "dennis", "zakary", "louvenia", "lew", "crawford", "caitlyn"]

# Write Ruby code to find out the answers to the following questions:

# * How would you print out each name backwards in `array`?
array.each do |name|
  puts name.reverse
end
# * What are the total number of characters in the names in `array`?
array.each do |name|
  puts name.length
end
# * How many names in `array` are less than 5 characters long?
number = 0
array.each do |name|
  if name.length < 5
    number += 1
  end
end
puts number
# * How many names in `array` end in a vowel?
number = 0
array.each do |name|
  if ("a" "e" "i" "o" "u" "y").include? name[-1]
    number += 1
  end
end
puts number
# * How many names in `array` are more than 5 characters long?
number = 0
array.each do |name|
  if name.length > 5
    number += 1
  end
end
puts number
# * How many names in `array` are exactly 5 characters in length?
number = 0
array.each do |name|
  if name.length == 5
    number += 1
  end
end
puts number
